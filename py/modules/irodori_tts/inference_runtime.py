...
class RuntimeKey:
    ...
    def __post_init__(self) -> None:
        paths: list[str] = []
        if self.lora_path:
            paths.append(str(self.lora_path))
        if self.lora_paths is not None:
            paths.extend(str(path) for path in self.lora_paths if str(path).strip())
        seen: set[str] = set()
        ...
        object.__setattr__(self, "lora_path", None)
        object.__setattr__(self, "lora_paths", tuple(unique_paths))
...
def _lora_adapter_name(index: int) -> str:
    return f"lora_{index}"


def _capture_lora_base_scaling(
    ...
    for module_name, module in model.named_modules():
        ...
        for adapter_name, value in getattr(module, "scaling", {}).items():
            base_scaling[(module_name, str(adapter_name))] = float(value)
    return base_scaling


def _set_active_lora_adapters(model: torch.nn.Module, adapter_names: tuple[str, ...]) -> None:
    if not adapter_names:
        return
    base_model = getattr(model, "base_model", None)
    if base_model is not None and hasattr(base_model, "set_adapter"):
        base_model.set_adapter(list(adapter_names), inference_mode=True)
        if hasattr(model, "active_adapter"):
            model.active_adapter = list(adapter_names)
        return
    if len(adapter_names) == 1 and hasattr(model, "set_adapter"):
        model.set_adapter(adapter_names[0])
        return
    raise RuntimeError(
        "This PEFT model does not support multiple active LoRA adapters. "
        "Use a newer PEFT LoRA tuner or merge adapters before inference."
    )


def _resolve_lora_adapter_scales(
    *,
    ...
    if not adapter_names:
        return {}
    if lora_scales:
        if len(lora_scales) != len(adapter_names):
            raise ValueError(
                f"lora_scales length ({len(lora_scales)}) must match loaded LoRA adapter count ({len(adapter_names)})."
            )
        return {
            adapter_name: float(scale)
            for adapter_name, scale in zip(adapter_names, lora_scales, strict=True)
        }
    return {adapter_name: float(lora_scale) for adapter_name in adapter_names}
...
def _apply_lora_settings(
    ...
    for module_name, module in model.named_modules():
        if not hasattr(module, "scaling"):
            continue
        for adapter_name, user_scale in adapter_scales.items():
            key = (module_name, adapter_name)
            if key not in base_scaling:
                continue
            effective_scale = 0.0 if module_name in disabled_set else base_scaling[key] * user_scale
    ...
    if applied == 0:
        ...
        warnings.warn(
            "LoRA adapters are loaded but no matching LoRA scaling layers were found in the model.",
            stacklevel=3,
        )


def _restore_lora_defaults(
    model: torch.nn.Module,
    base_scaling: dict[tuple[str, str], float],
) -> None:
    for module_name, module in model.named_modules():
        if hasattr(module, "scaling"):
            for adapter_name in list(getattr(module, "scaling", {})):
                ...
                if key in base_scaling:
                    module.scaling[adapter_name] = base_scaling[key]
...
class InferenceRuntime:
    def __init__(
        ...
        *,
        ...
        codec: DACVAECodec,
        watermarker: SilentCipherWatermarker,
        default_text_max_len: int,
        default_caption_max_len: int,
        lora_adapter_names: tuple[str, ...] = (),
        lora_base_scaling: dict[tuple[str, str], float] | None = None,
    ) -> None:
        self.key = key
    ...
    def from_key(cls, key: RuntimeKey) -> InferenceRuntime:
        ...
        lora_adapter_names: tuple[str, ...] = ()
        lora_base_scaling: dict[tuple[str, str], float] = {}
        if key.lora_paths:
            try:
            ...
            lora_cache_dir = _extension_data_dir() / "loras"
            lora_cache_dir.mkdir(parents=True, exist_ok=True)
            adapter_names: list[str] = []
            for i, lora_path in enumerate(key.lora_paths):
                adapter_name = _lora_adapter_name(i)
                lora_warnings, lora_origin, _ = _validate_lora_adapter(
                    lora_path,
                    model_cfg,
                )
                print(f"[lora] adapter {i}: origin={lora_origin}", flush=True)
                print(f"[lora] adapter {i}: path={lora_path}", flush=True)
            ...
            lora_adapter_names = tuple(adapter_names)
            _set_active_lora_adapters(model, lora_adapter_names)
            model = model.to(dtype=model_dtype)
            model.eval()
            lora_base_scaling = _capture_lora_base_scaling(model)
            print(f"[lora] loaded adapters: {', '.join(lora_adapter_names)}", flush=True)
        ...
        return cls(
            ...
            lora_base_scaling=lora_base_scaling,
        )
    ...
    def synthesize(
        ...
        *,
        ...
        if self.key.lora_paths:
            adapter_scales = _resolve_lora_adapter_scales(
                adapter_names=self.lora_adapter_names,
                lora_scale=float(req.lora_scale),
                lora_scales=tuple(float(scale) for scale in req.lora_scales),
            )
            _log(
                "[runtime] lora_paths={} scales={}".format(
                    list(self.key.lora_paths),
                    adapter_scales,
                )
            )
        else:
            ...
            try:
                ...
                if self.key.silentcipher_watermark_enabled:
                    if self.watermarker.ready:
                        ...
                        msg = (
                            "warning: SilentCipher watermark is enabled but unavailable; "
                            "generated audio was not watermarked."
                        )
                        messages.append(msg)
                        _log(msg)
                else:
                    msg = "info: SilentCipher watermark is disabled by model_config."
                    messages.append(msg)
                    _log(msg)
                ...
                if lora_active:
                    _restore_lora_defaults(self.model, self.lora_base_scaling)
