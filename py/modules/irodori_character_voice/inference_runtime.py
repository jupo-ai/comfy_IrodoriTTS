...
from .projector import infer_projector_config_from_state_dict, projector_config_to_dict
...
def _request_comfy_free_memory(memory_required: int, device: torch.device) -> None:
    ...
    try:
        ...
        print(
            f"[Irodori Character Voice] warning: ComfyUI memory free request failed: {exc}",
            flush=True,
        )
...
class RuntimeKey:
    ...
    character_image_size: int | None = None
...
class SamplingRequest:
    ...
    character_image: str | Path | torch.Tensor | None = None
    num_steps: int = 40
...
def resolve_cfg_scales(
    *,
    ...
    if not use_character_condition:
        if character_val > 0.0:
            messages.append(
                "info: character conditioning is disabled for this request; ignoring cfg_scale_character."
            )
        character_val = 0.0
...
_DEFAULT_CHARACTER_ENCODER_MODEL = "hf_hub:SmilingWolf/wd-vit-tagger-v3"
_STATE_INFERRED_CONFIG_KEYS = (
    "latent_dim",
    "model_dim",
    "num_layers",
    "num_heads",
...
def _shape(state: dict[str, torch.Tensor], key: str) -> tuple[int, ...] | None:
    value = state.get(key)
...
def _count_indexed_modules(state: dict[str, torch.Tensor], prefix: str) -> int:
    count = 0
    for key in state:
        ...
        tail = key[len(prefix) :]
        try:
            count = max(count, int(tail.split(".", 1)[0]) + 1)
        except (IndexError, ValueError):
            pass
    return count


def _ratio_from_weight(
    state: dict[str, torch.Tensor], key: str, dim: int, fallback: float
) -> float:
    shape = _shape(state, key)
    if shape is None or len(shape) != 2 or dim <= 0:
        return float(fallback)
    return float(shape[0]) / float(dim)


def _heads_from_norm(state: dict[str, torch.Tensor], key: str, fallback: int) -> int:
    shape = _shape(state, key)
...
def _tokenizer_repo_for_vocab(vocab_size: int, fallback: str) -> str:
    if int(vocab_size) == 99574:
        return "llm-jp/llm-jp-3-150m"
    if int(vocab_size) == 102400:
        return "sbintuitions/sarashina2.2-0.5b"
    return fallback


def _checkpoint_uses_character_condition(state: dict[str, torch.Tensor], cfg: dict | None) -> bool:
    if isinstance(cfg, dict) and bool(cfg.get("use_character_condition", False)):
        return True
    return any(
...
def _infer_model_config_from_state_dict(
    ...
    warnings: list[str] = [
        "warning: checkpoint metadata did not include complete config_json/model_config; "
        "inferring model architecture from state_dict where possible."
    ]
    base = ModelConfig()
    cfg: dict[str, object] = {}

    in_proj = _shape(state, "in_proj.weight")
    if in_proj is not None and len(in_proj) == 2:
        cfg["model_dim"] = int(in_proj[0])
        cfg["latent_dim"] = int(in_proj[1])
        cfg["latent_patch_size"] = 1
    model_dim = int(cfg.get("model_dim", base.model_dim))
    cfg["num_layers"] = _count_indexed_modules(state, "blocks.") or base.num_layers
    cfg["num_heads"] = _heads_from_norm(state, "blocks.0.attention.q_norm.weight", base.num_heads)
    cfg["mlp_ratio"] = _ratio_from_weight(
        state, "blocks.0.mlp.w1.weight", model_dim, base.mlp_ratio
    )

    text_embedding = _shape(state, "text_encoder.text_embedding.weight")
    if text_embedding is not None and len(text_embedding) == 2:
        cfg["text_vocab_size"] = int(text_embedding[0])
        cfg["text_dim"] = int(text_embedding[1])
        cfg["text_tokenizer_repo"] = _tokenizer_repo_for_vocab(
            int(text_embedding[0]),
            base.text_tokenizer_repo,
        )
    text_dim = int(cfg.get("text_dim", base.text_dim))
    cfg["text_layers"] = _count_indexed_modules(state, "text_encoder.blocks.") or base.text_layers
    cfg["text_heads"] = _heads_from_norm(
        state,
        "text_encoder.blocks.0.attention.q_norm.weight",
        base.text_heads,
    )
    cfg["text_mlp_ratio"] = _ratio_from_weight(
        state,
        "text_encoder.blocks.0.mlp.w1.weight",
        text_dim,
        base.text_mlp_ratio_resolved,
    )

    caption_embedding = _shape(state, "caption_encoder.text_embedding.weight")
    if caption_embedding is not None and len(caption_embedding) == 2:
        cfg["use_caption_condition"] = True
        cfg["caption_vocab_size"] = int(caption_embedding[0])
        cfg["caption_dim"] = int(caption_embedding[1])
        cfg["caption_tokenizer_repo"] = _tokenizer_repo_for_vocab(
            int(caption_embedding[0]),
            str(cfg.get("text_tokenizer_repo", base.text_tokenizer_repo)),
        )
        caption_dim = int(caption_embedding[1])
        cfg["caption_layers"] = (
            _count_indexed_modules(state, "caption_encoder.blocks.") or base.text_layers
        )
        cfg["caption_heads"] = _heads_from_norm(
            state,
            "caption_encoder.blocks.0.attention.q_norm.weight",
            base.text_heads,
        )
        cfg["caption_mlp_ratio"] = _ratio_from_weight(
            state,
    ...
    speaker_in = _shape(state, "speaker_encoder.in_proj.weight")
    if speaker_in is not None and len(speaker_in) == 2:
        patched_latent_dim = int(cfg.get("latent_dim", base.patched_latent_dim))
        cfg["speaker_dim"] = int(speaker_in[0])
        cfg["speaker_patch_size"] = max(1, int(speaker_in[1]) // max(1, patched_latent_dim))
        cfg["speaker_layers"] = (
            _count_indexed_modules(state, "speaker_encoder.blocks.") or base.speaker_layers
        )
        cfg["speaker_heads"] = _heads_from_norm(
            state,
            "speaker_encoder.blocks.0.attention.q_norm.weight",
            base.speaker_heads,
        )
        cfg["speaker_mlp_ratio"] = _ratio_from_weight(
            state,
            "speaker_encoder.blocks.0.mlp.w1.weight",
            int(speaker_in[0]),
            base.speaker_mlp_ratio_resolved,
        )

    timestep = _shape(state, "cond_module.0.weight")
    if timestep is not None and len(timestep) == 2:
        cfg["timestep_embed_dim"] = int(timestep[1])
    adaln = _shape(state, "blocks.0.attention_adaln.shift_down.weight")
    if adaln is not None and len(adaln) == 2:
        cfg["adaln_rank"] = int(adaln[0])

    return cfg, warnings


def _prepare_model_config_for_inference(
    *,
    state: dict[str, torch.Tensor],
    raw_cfg: dict | None,
    key: RuntimeKey,
    ...
    if isinstance(raw_cfg, dict) and raw_cfg:
        cfg = dict(raw_cfg)
        if any(key not in cfg for key in _STATE_INFERRED_CONFIG_KEYS):
            inferred_cfg, inferred_warnings = _infer_model_config_from_state_dict(state)
            warnings.extend(inferred_warnings)
            inferred_cfg.update(cfg)
            cfg = inferred_cfg
    else:
        cfg, inferred_warnings = _infer_model_config_from_state_dict(state)
        warnings.extend(inferred_warnings)

    checkpoint_has_character = _checkpoint_uses_character_condition(state, cfg)
    if checkpoint_has_character:
        cfg["use_character_condition"] = True
        cfg["character_attention_mode"] = str(cfg.get("character_attention_mode") or "joint")
        cfg["character_use_all_patches"] = bool(cfg.get("character_use_all_patches", True))
        image_size = key.character_image_size or cfg.get("character_image_size") or 448
        cfg["character_image_size"] = int(image_size)

        if key.character_encoder_model is not None:
            cfg["character_encoder_model"] = str(key.character_encoder_model)
        elif not cfg.get("character_encoder_model"):
            cfg["character_encoder_model"] = _DEFAULT_CHARACTER_ENCODER_MODEL
            warnings.append(
                "warning: character_encoder_model was missing from checkpoint metadata; "
                f"using default {cfg['character_encoder_model']!r}."
            )

        projector_cfg, projector_in_dim, projector_out_dim, projector_warnings = (
            infer_projector_config_from_state_dict(
                state,
                fallback=cfg.get("character_projector"),
            )
        )
        warnings.extend(projector_warnings)
        if projector_cfg is not None:
            inferred_projector = projector_config_to_dict(projector_cfg)
            if (
                cfg.get("character_projector")
                and cfg.get("character_projector") != inferred_projector
            ):
                warnings.append(
                    "warning: character_projector metadata does not match state_dict; "
                    "using state_dict-inferred projector config."
                )
            cfg["character_projector"] = inferred_projector
        if projector_out_dim is not None:
            if (
                cfg.get("character_dim") is not None
                and int(cfg["character_dim"]) != projector_out_dim
            ):
                warnings.append(
                    "warning: character_dim metadata does not match projector output; "
                    f"using state_dict value {projector_out_dim}."
                )
            cfg["character_dim"] = int(projector_out_dim)
        else:
            norm_shape = _shape(state, "character_encoder.norm.weight")
            if norm_shape is not None and len(norm_shape) == 1:
                cfg["character_dim"] = int(norm_shape[0])
            else:
                wk_shape = _shape(state, "blocks.0.attention.wk_character.weight")
                if wk_shape is not None and len(wk_shape) == 2:
                    cfg["character_dim"] = int(wk_shape[1])

        if key.character_hidden_state_index is not None:
            cfg["character_hidden_state_index"] = int(key.character_hidden_state_index)
        elif cfg.get("character_hidden_state_index") is None and projector_in_dim is not None:
            from .image_encoder import resolve_character_hidden_state_index

            resolved_index, hidden_warnings = resolve_character_hidden_state_index(
                str(cfg["character_encoder_model"]),
                image_size=int(cfg["character_image_size"]),
                expected_dim=int(projector_in_dim),
                preferred_index=None,
            )
            warnings.extend(hidden_warnings)
            cfg["character_hidden_state_index"] = resolved_index
            if resolved_index is not None:
                warnings.append(
                    "warning: character_hidden_state_index was inferred from projector input "
                    f"dim={projector_in_dim}: {resolved_index}."
                )
...
def _load_checkpoint_from_pt(path: Path) -> tuple[dict[str, torch.Tensor], dict, dict | None]:
    ...
    return (
        ...
        train_cfg if isinstance(train_cfg, dict) else None,
    )
...
def _load_checkpoint_from_safetensors(
    ...
    model_cfg, inference_cfg = read_safetensors_metadata_config(metadata, path=path)
...
class InferenceRuntime:
    def __init__(
        ...
        *,
        ...
        load_messages: list[str] | None = None,
    ) -> None:
        self.key = key
        ...
        self.load_messages = list(load_messages or [])
    ...
    def from_key(cls, key: RuntimeKey) -> InferenceRuntime:
        ...
        model_cfg_dict, load_messages = _prepare_model_config_for_inference(
            state=model_state,
        ...
        tokenizer_cache_dir = _extension_data_dir() / "tokenizers"
        ...
        if model_cfg.use_character_condition:
            character_image_transform = build_character_transform(
                str(model_cfg.character_encoder_model),
                int(model_cfg.character_image_size),
            )

        codec_repo_safe = str(key.codec_repo).replace("/", "_")
        codec_cache_dir = (
    ...
    def synthesize(
        ...
        *,
        ...
        messages: list[str] = list(self.load_messages)
        ...
        has_character_image = bool(
            self.model_cfg.use_character_condition
            and req.character_image is not None
            and (
                isinstance(req.character_image, torch.Tensor)
                or str(req.character_image).strip() != ""
            )
        )
        ...
        (
            ...
            use_character_condition=has_character_image,
        )
            ...
            if self.model_cfg.use_character_condition:
                ...
                if has_character_image:
                    if self.character_image_transform is None:
                        raise RuntimeError(
                            "Character conditioning is enabled but image transform is not loaded."
                        )
                    if isinstance(req.character_image, torch.Tensor):
                        image = coerce_character_image(req.character_image)
                    else:
                        image = coerce_character_image(Path(str(req.character_image)).expanduser())
                    image_tensor = self.character_image_transform(image).unsqueeze(0)
                    character_images = image_tensor.expand(num_candidates, -1, -1, -1).to(
                        self.model_device
                    )
                    character_mask = torch.ones(
                        (num_candidates, 1), dtype=torch.bool, device=self.model_device
                    )
                else:
                ...
                character_images = character_images.to(dtype=next(self.model.parameters()).dtype)
            stage_sec = _measure_end(self.model_device, t0)
            if self.model_cfg.use_character_condition:
                stage_timings.append(("prepare_character", stage_sec))
                _log(f"[runtime] prepare_character: {stage_sec * 1000.0:.1f} ms")
