...
class JointAttention(nn.Module):
    ...
    def forward(
        ...
        if self.has_character_condition and self.character_attention_mode == "cross":
            if k_character is None or v_character is None or character_mask is None:
                raise RuntimeError("Character projections are missing for cross attention.")
            if bool(character_mask.any().item()):
                y_character = F.scaled_dot_product_attention(
                    q.transpose(1, 2),
...
class DiffusionBlock(nn.Module):
    def __init__(self, cfg: ModelConfig):
        ...
        self.attention = JointAttention(
            ...
            cfg.character_dim_resolved if cfg.use_character_condition else None,
            norm_eps=cfg.norm_eps,
...
class TextToLatentRFDiT(nn.Module):
    ...
    def encode_conditions(
        ...
        if self.cfg.use_character_condition and self.character_encoder is None:
            raise RuntimeError("Character conditioning is enabled but character encoder is missing.")
        ...
        if self.cfg.use_character_condition:
            assert self.character_encoder is not None
            batch_size = int(text_input_ids.shape[0])
            if character_images is None:
                param = next(self.character_encoder.parameters())
                character_images = torch.zeros(
                    (
                        batch_size,
                        3,
                        self.cfg.character_image_size,
                        self.cfg.character_image_size,
                    ),
            ...
            if character_mask is None:
                ...
                character_mask = character_mask.to(device=character_state.device, dtype=torch.bool)
                if character_mask.ndim != 2 or character_mask.shape[0] != character_state.shape[0]:
                    raise ValueError(
                        "character_mask must have shape (B, N) matching character images."
                    )
                if character_mask.shape[1] == 1 and character_state.shape[1] != 1:
                    character_mask = character_mask.expand(-1, character_state.shape[1])
                elif character_mask.shape[1] != character_state.shape[1]:
                    raise ValueError(
                        "character_mask length must be 1 or match encoded character tokens: "
                        f"mask={tuple(character_mask.shape)} state={tuple(character_state.shape)}"
                    )
            if character_condition_dropout is not None:
        ...
        return EncodedConditions(
            ...
            character_mask=character_mask,
            character_noisy_state=character_noisy_state,
        )
